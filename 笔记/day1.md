...
路由组件:Home、Search、Login（有Header，没有底部的Footer组件，带有二维码的）、Register（有Header，没有底部的Footer组件，带二维码的）
...
5.3：注册完路由，所有组件都会有$router、$route属性  
...
通过路由元信息，this.$route.meta.showFooter 决定是否显示
...
路由传递参数先关面试题
     ...
     2:如何指定params参数可传可不传?（配置路由时候占位了params参数，但路由跳转时不传递，路径会出现问题）
        http://localhost:8080/#/search
        http://localhost:8080/#/?k=SB
        在配置路由的时候在path后面加上？，代表可传可不传
